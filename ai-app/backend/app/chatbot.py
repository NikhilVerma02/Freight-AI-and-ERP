...
and the current ERP system state. Uses Groq for generation with full per-session
...
def ask(username: str, session_id: str, question: str) -> dict:
    ...
    if client is None:
        ...
        try:
            from app.config.agents import GROQ_CHAT_MODEL
            ...
            logger.error("chatbot ask failed: %s", exc)
            err = str(exc)
