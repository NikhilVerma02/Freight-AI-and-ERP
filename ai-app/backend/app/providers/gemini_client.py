"""
Gemini multimodal client — used exclusively by the Inspector Agent to
understand uploaded video/image/audio evidence of freight damage in a
single call (no separate ffmpeg/transcription/vision steps needed: Gemini
accepts video, image, and audio bytes inline in one generate_content call).

The free tier intermittently returns 503 UNAVAILABLE ("high demand") for a
given model with no actual problem on our end — retried with backoff, then
failed over to the next candidate model, before giving up.
"""
from __future__ import annotations

import logging
import time

from app import observability
from app.config.agents import GEMINI_API_KEY, GEMINI_MULTIMODAL_MODEL
from app.providers import envelope

logger = logging.getLogger("ai_app.providers.gemini")

_client = None

# Tried in order after GEMINI_MULTIMODAL_MODEL itself. All multimodal-capable,
# free-tier models — a transient 503 on one rarely hits all of them at once.
_FALLBACK_MODELS = ["gemini-2.5-flash-lite", "gemini-2.0-flash-lite", "gemini-flash-lite-latest"]

_RETRYABLE_MARKERS = ("503", "UNAVAILABLE", "RESOURCE_EXHAUSTED", "429")


def _get_client():
    global _client
    if not GEMINI_API_KEY:
        return None
    if _client is None:
        from google import genai

        _client = genai.Client(api_key=GEMINI_API_KEY)
    return _client


def _is_retryable(exc: Exception) -> bool:
    text = str(exc)
    return any(marker in text for marker in _RETRYABLE_MARKERS)


def multimodal_extract(system_prompt: str, user_text: str, files: list[dict], trace_id: str | None = None) -> dict:
    """files: list of {"data": bytes, "mime_type": str}. Returns a provider envelope
    whose `content` is the model's raw text response on success.
    trace_id (optional): Langfuse trace to nest this call under — see app/observability.py."""
    client = _get_client()
    if client is None:
        return envelope(GEMINI_MULTIMODAL_MODEL, 0.0, "error", error="Gemini client not configured (missing GEMINI_API_KEY)")

    from google.genai import types

    parts: list = [user_text]
    for f in files:
        parts.append(types.Part.from_bytes(data=f["data"], mime_type=f["mime_type"]))
    config = types.GenerateContentConfig(system_instruction=system_prompt, temperature=0)

    generation = observability.start_generation(
        trace_id,
        "gemini_multimodal_extract",
        GEMINI_MULTIMODAL_MODEL,
        input={"system_prompt": system_prompt, "user_text": user_text, "file_count": len(files)},
    )

    candidates = [GEMINI_MULTIMODAL_MODEL] + [m for m in _FALLBACK_MODELS if m != GEMINI_MULTIMODAL_MODEL]
    last_error: Exception | None = None
    overall_start = time.perf_counter()

    for model in candidates:
        # One retry per model (transient overload often clears in a couple seconds),
        # then move on to the next candidate model rather than retrying it forever.
        for attempt in range(2):
            try:
                resp = client.models.generate_content(model=model, contents=parts, config=config)
                latency_ms = (time.perf_counter() - overall_start) * 1000
                usage = getattr(resp, "usage_metadata", None)
                prompt_tokens = getattr(usage, "prompt_token_count", None) if usage else None
                completion_tokens = getattr(usage, "candidates_token_count", None) if usage else None
                observability.finish_generation(
                    generation, output=resp.text, status="ok", usage={"input": prompt_tokens, "output": completion_tokens}
                )
                return envelope(model, latency_ms, "ok", resp.text, prompt_tokens, completion_tokens)
            except Exception as exc:
                last_error = exc
                if not _is_retryable(exc):
                    logger.error("multimodal_extract: non-retryable error on model %s: %s", model, exc)
                    break  # don't bother retrying/falling over for a non-transient error (e.g. bad input)
                logger.warning("multimodal_extract: model %s attempt %d failed (%s)", model, attempt + 1, exc)
                if attempt == 0:
                    time.sleep(1.5)
        else:
            continue  # both attempts retryable-failed for this model — try the next model
        if not _is_retryable(last_error):
            break  # non-retryable: stop trying other models too

    latency_ms = (time.perf_counter() - overall_start) * 1000
    logger.error("multimodal_extract failed on all candidate models: %s", last_error)
    observability.finish_generation(generation, status="error", error=str(last_error))
    return envelope(GEMINI_MULTIMODAL_MODEL, latency_ms, "error", error=str(last_error))
