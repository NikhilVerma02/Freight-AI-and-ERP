"""
Vector store wrapper. Tries chromadb (persistent client under
ai-app/backend/data/vector_cache/) first; falls back to a pure-numpy
cosine-similarity store transparently if chromadb is unavailable/fails to
import — keeps the rest of the app working in constrained sandboxes.

This is the "Document RAG" backing store: per-vendor SLA-clause chunks +
their embeddings, queried by app/rag/policy_rag.py. (The other RAG type in
this app — tool/API-grounded retrieval over live ERP state via MCP — does
not use embeddings and has no vector store; see app/mcp_client.py.)
"""
...
logger = logging.getLogger("ai_app.rag.vector_store")

VECTOR_CACHE_DIR = Path(__file__).parent.parent.parent / "data" / "vector_cache"
...
class _NumpyCollection:
    """Minimal numpy-backed fallback collection, persisted as .npz + json sidecar."""
...
def collection_count(collection_name: str) -> int:
    """Return how many chunks are currently stored for this collection (0 if none/missing)."""
    if _USE_CHROMA:
        try:
...
def query(collection_name: str, query_embedding: list[float], top_k: int = 4) -> dict[str, Any]:
    """Returns {'documents': [...], 'metadatas': [...], 'distances': [...]} (top_k results)."""
    if _USE_CHROMA:
        try:
            coll = _chroma_client.get_or_create_collection(collection_name)
            if coll.count() == 0:
