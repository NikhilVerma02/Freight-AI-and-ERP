...
function renderMarkdown(text: string): React.ReactNode {
  ...
  for (const line of lines) {
    if (line.startsWith("```")) {
      if (inCode) {
        elements.push(
          <pre key={key++} className="my-2 overflow-x-auto rounded-lg bg-slate-900/80 border border-slate-700/50 p-3 text-xs font-mono text-slate-300">
            {codeLines.join("\n")}
          </pre>
        );
        codeLines = [];
      ...
      elements.push(<h3 key={key++} className="mt-3 mb-1 text-sm font-semibold text-slate-100">{line.slice(4)}</h3>);
    } else if (line.startsWith("## ")) {
      elements.push(<h2 key={key++} className="mt-3 mb-1 text-sm font-bold text-slate-50">{line.slice(3)}</h2>);
    } else if (line.match(/^[-*] /)) {
      elements.push(
        <li key={key++} className="ml-4 list-disc text-sm leading-relaxed text-slate-300">
          {inlineFormat(line.slice(2))}
        </li>
      );
    } else if (line.match(/^\d+\. /)) {
      elements.push(
        <li key={key++} className="ml-4 list-decimal text-sm leading-relaxed text-slate-300">
...
export default function Chatbot() {
  ...
  return (
    <div className="flex h-full overflow-hidden rounded-xl border border-slate-800/60 bg-slate-950">
      ...
      <aside className="flex w-64 flex-shrink-0 flex-col border-r border-slate-800/80 bg-slate-900/50">
        ...
        <div className="flex items-center gap-2 border-b border-slate-800/60 px-3 py-3">
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 transition-colors hover:bg-slate-800 hover:text-accent-300"
          >
          ...
          <div className="relative flex-1">
            <input
              ...
              placeholder="Search"
              className="w-full rounded-lg border border-slate-700/60 bg-slate-800/60 py-1.5 pl-7 pr-2 text-xs text-slate-300 placeholder-slate-500 outline-none focus:border-accent-500/50 focus:ring-0"
            />
      ...
      <div className="relative flex flex-1 flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          {!activeSession || activeSession.messages.length === 0 ? (
            /* Empty state — agent icon + greeting */
              ...
              <div className="flex flex-wrap justify-center gap-2 max-w-lg">
                {[
                  ...
                  <button
                    ...
                    className="rounded-full border border-slate-700/60 bg-slate-800/60 px-3 py-1.5 text-xs text-slate-400 transition-colors hover:border-accent-500/40 hover:bg-slate-800 hover:text-accent-300"
                  >
                    {suggestion}
                  </button>
                ))}
            ...
            <div className="mx-auto max-w-3xl space-y-6 px-6 py-8">
              {activeSession.messages.map((msg, i) => (
                <div key={i} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                  {/* Avatar */}
                  {msg.role === "assistant" ? (
                    <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg border border-accent-500/30 bg-slate-800 text-sm">
                      ⌬
                    </div>
                  ) : (
                  ...
                  <div
                    className={`max-w-[78%] rounded-2xl px-4 py-3 ${
                      msg.role === "user"
                        ? "rounded-tr-sm bg-accent-500/15 border border-accent-500/20 text-slate-200"
        ...
        <div className="relative px-4 pb-4 pt-2">
          ...
          <div className="relative mx-auto max-w-3xl">
            {/* Outer glass shell */}
            <div className="relative rounded-2xl border border-slate-700/60 bg-slate-800/40 shadow-[0_8px_32px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.05)] backdrop-blur-xl">
              ...
              <div className="flex items-end gap-2 px-4 py-3">
                ...
                <textarea
                  ...
                  rows={1}
                  placeholder="Ask FreightBot anything about your freight operations…"
                  disabled={loading}
              ...
              <div className="px-4 pb-2 text-[10px] text-slate-600">
                Press <kbd className="rounded border border-slate-700 px-1 font-mono">Enter</kbd> to send · <kbd className="rounded border border-slate-700 px-1 font-mono">Shift+Enter</kbd> for new line
              </div>
            </div>
          </div>
        </div>
