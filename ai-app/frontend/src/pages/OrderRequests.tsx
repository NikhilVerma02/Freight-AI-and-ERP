...
export default function OrderRequests() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-slate-100">{t("orderRequests.title")}</h1>
        <p className="mt-1 text-sm text-slate-400">{t("orderRequests.subtitle")}</p>
      </div>
            ...
            <motion.div key={o.id} variants={fadeUpItem}>
              <Card className="flex flex-col gap-2 border-t-2 p-4" style={{ borderTopColor: AGENT_COLORS.reorder.hex }}>
                ...
                <div className="text-xs text-slate-500">
                  {o.customer_username} → {o.vendor_username}
                </div>
