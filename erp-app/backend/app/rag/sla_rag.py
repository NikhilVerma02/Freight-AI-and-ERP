"""
RAG orchestration for vendor SLA documents: chunk + embed (Gemini) + store
(chromadb, see vector_store.py) on upload, then retrieve top-k chunks +
generate a grounded answer (Groq) on question-asking.
"""
...
def index_sla(sla_id: int, vendor_username: str, text: str) -> dict:
    ...
    if embeddings is None:
        return {"indexed": False, "chunk_count": 0, "error": "Embedding failed (check GEMINI_API_KEY)"}
...
def ask_sla(sla_id: int, question: str, top_k: int = SLA_RAG_TOP_K) -> dict:
    ...
    if query_embedding is None:
        return {"answer": None, "sources": [], "error": "Embedding the question failed (check GEMINI_API_KEY)"}
    ...
    if answer is None:
        return {"answer": None, "sources": sources, "error": "Answer generation failed (check GROQ_API_KEY)"}
