...
async def upload_sla(
    ...
    if not requested_customers:
        raise HTTPException(status_code=400, detail="Select at least one customer to share this SLA with")
    ...
    dest_filename = f"{vendor_username}_{int(time.time() * 1000)}.pdf"
...
def delete_sla(sla_id: int, current_user: dict = Depends(require_role("vendor", "admin"))):
    record = sla_svc.get_sla_by_id(sla_id)
    if not record:
        raise HTTPException(status_code=404, detail="No SLA document on file")
    if current_user["role"] == "vendor" and record.get("vendor_username") != current_user["username"]:
...
def my_customers(vendor_username: str, current_user: dict = Depends(get_current_user)):
    if current_user["role"] == "vendor" and current_user["username"] != vendor_username:
        raise HTTPException(status_code=403, detail="Not permitted")
