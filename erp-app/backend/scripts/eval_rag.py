...
Ragas (free, open-source) with Groq as the judge LLM and Gemini for
embeddings — both already configured for this project, no paid service.
...
langchain-core/-community/-openai/-groq generation than other tools in this
...
one-time setup below):
    ...
    .venv-eval/Scripts/python -m pip install "langchain-community==0.3.27" \
        "langchain-groq==0.3.8" "langchain-google-genai==2.1.12" "langchain-openai==0.3.35"
...
real ask_sla() RAG pipeline (Gemini embeddings + Groq answer) and scores
the result with four Ragas metrics:
...
from app.config import GEMINI_API_KEY, GEMINI_EMBEDDING_MODEL, GROQ_API_KEY, GROQ_CHAT_MODEL  # noqa: E402
...
def main() -> None:
    if not GROQ_API_KEY or not GEMINI_API_KEY:
        print("ERROR: GROQ_API_KEY and GEMINI_API_KEY must be set in .env to run this eval.")
        sys.exit(1)
    ...
    llm = LangchainLLMWrapper(ChatGroq(model=GROQ_CHAT_MODEL, api_key=GROQ_API_KEY, temperature=0))
    embeddings = LangchainEmbeddingsWrapper(GoogleGenerativeAIEmbeddings(model=GEMINI_EMBEDDING_MODEL, google_api_key=GEMINI_API_KEY))

    print("\nRunning Ragas evaluation (Groq judge + Gemini embeddings)...")
