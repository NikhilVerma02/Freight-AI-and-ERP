...
function renderMd(text: string): React.ReactNode {
  ...
  const inline = (line: string) =>
    line.split(/(\*\*[^*]+\*\*|`[^`]+`)/g).map((p, i) => {
      ...
      if (p.startsWith("`") && p.endsWith("`"))
        return <code key={i} className="rounded bg-slate-700/60 px-1 py-0.5 text-xs font-mono text-cyan-300">{p.slice(1,-1)}</code>;
      return p;
    });
  for (const line of lines) {
    if (line.startsWith("```")) {
      if (inCode) {
        out.push(<pre key={key++} className="my-2 overflow-x-auto rounded-lg bg-slate-900 border border-slate-700/50 p-3 text-xs font-mono text-slate-300">{codeLines.join("\n")}</pre>);
        codeLines = []; inCode = false;
      } else { inCode = true; }
    } else if (inCode) {
      codeLines.push(line);
    } else if (line.startsWith("## ")) {
      out.push(<h2 key={key++} className="mt-3 mb-1 text-sm font-bold text-slate-100">{line.slice(3)}</h2>);
    } else if (line.match(/^[-*] /)) {
      out.push(<li key={key++} className="ml-4 list-disc text-sm leading-relaxed text-slate-300">{inline(line.slice(2))}</li>);
    } else if (line.trim() === "") {
      out.push(<div key={key++} className="h-2" />);
    } else {
      out.push(<p key={key++} className="text-sm leading-relaxed text-slate-300">{inline(line)}</p>);
    }
  }
...
export default function Chatbot() {
  ...
  function handleClearAll() {
    // delete each session individually (no bulk endpoint in erp-app yet)
  ...
  return (
    <div className="relative flex h-[calc(100vh-7.5rem)] overflow-hidden rounded-xl border border-slate-800/60 bg-slate-950">

      {/* ── Sidebar ──────────────────────────────────────────────────────────── */}
      <aside className="flex w-64 flex-shrink-0 flex-col border-r border-slate-800/80 bg-slate-900/50">

        {/* Top controls */}
        <div className="flex items-center gap-2 border-b border-slate-800/60 px-3 py-3">
          <button
            onClick={startNewChat}
            title="New conversation"
            className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 transition-colors hover:bg-slate-800 hover:text-cyan-300"
          >
          ...
          <div className="relative flex-1">
            <input
              ...
              placeholder="Search"
              className="w-full rounded-lg border border-slate-700/60 bg-slate-800/60 py-1.5 pl-7 pr-2 text-xs text-slate-300 placeholder-slate-500 outline-none focus:border-cyan-500/50"
            />
          ...
          {loadingSessions ? (
            ...
            grouped.map((g) => (
              <div key={g.label} className="mb-3">
                <p className="px-3 pb-1 text-[10px] font-semibold uppercase tracking-widest text-slate-600">{g.label}</p>
                {g.items.map((s) => (
                  <div key={s.id} className="relative px-2">
                    ...
                    {menuOpen === s.id && (
                      <div ref={menuRef} className="absolute right-3 top-8 z-50 min-w-[120px] rounded-lg border border-slate-700 bg-slate-900 py-1 shadow-xl">
                        <button
      ...
      <div className="relative flex flex-1 flex-col overflow-hidden">
          ...
          {loadingMessages ? (
            ...
            <div className="flex h-full flex-col items-center justify-center gap-6 px-8 text-center">
              ...
              <div className="relative">
                <div className="flex h-20 w-20 items-center justify-center rounded-2xl border border-cyan-500/30 bg-gradient-to-br from-slate-800 to-slate-900 shadow-[0_0_40px_rgba(6,182,212,0.15)]">
                  <svg viewBox="0 0 40 40" fill="none" className="h-10 w-10">
                    <circle cx="20" cy="20" r="18" stroke="currentColor" strokeWidth="1.5" className="text-cyan-500/30"/>
                    <path d="M12 20h16M20 12v16" stroke="currentColor" strokeWidth="2" strokeLinecap="round" className="text-cyan-400"/>
                    <circle cx="20" cy="20" r="4" fill="currentColor" className="text-cyan-400"/>
                  </svg>
                </div>
                <span className="absolute -bottom-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-cyan-500 text-[8px] font-bold text-slate-950">AI</span>
              </div>

              <div>
                <h2 className="text-2xl font-semibold text-slate-100">{getGreeting(displayName)}</h2>
              ...
              <div className="flex flex-wrap justify-center gap-2 max-w-lg">
                {[
                  ...
                  <button
                    ...
                    className="rounded-full border border-slate-700/60 bg-slate-800/60 px-3 py-1.5 text-xs text-slate-400 transition-colors hover:border-cyan-500/40 hover:bg-slate-800 hover:text-cyan-300"
                  >
                    {s}
                  </button>
                ))}
            ...
            <div className="mx-auto max-w-3xl space-y-6 px-6 py-8">
              {messages.map((m, i) => (
                ...
                >
                  {/* Avatar */}
                  {m.role === "assistant" ? (
                    <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg border border-cyan-500/30 bg-slate-800">
                      <svg viewBox="0 0 20 20" fill="none" className="h-4 w-4">
                        <circle cx="10" cy="10" r="8" stroke="currentColor" strokeWidth="1.5" className="text-cyan-500/40"/>
                        <circle cx="10" cy="10" r="2.5" fill="currentColor" className="text-cyan-400"/>
                      </svg>
                    </div>
                  ) : (
                    <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-cyan-500/20 text-xs font-bold text-cyan-300">
                      {initials}
                    </div>
                  )}
                  ...
                  <div
                    className={`max-w-[78%] rounded-2xl px-4 py-3 ${
                      m.role === "user"
                        ? "rounded-tr-sm bg-cyan-500/15 border border-cyan-500/20 text-slate-200"
                        : m.error
                          ? "rounded-tl-sm border border-red-500/30 bg-red-900/20 text-red-300"
                          : "rounded-tl-sm bg-slate-800/70 border border-slate-700/50"
                    }`}
                  >
              ...
              {sending && (
                <div className="flex gap-3">
                  ...
                  <div className="rounded-2xl rounded-tl-sm border border-slate-700/50 bg-slate-800/70 px-4 py-3">
        ...
        <div className="relative px-4 pb-4 pt-2">
          ...
          <div className="pointer-events-none absolute inset-x-4 -top-6 h-8 bg-gradient-to-t from-slate-950 to-transparent" />

          <div className="relative mx-auto max-w-3xl">
            {/* Outer glass shell */}
            <div className="relative rounded-2xl border border-slate-700/60 bg-slate-800/40 shadow-[0_8px_32px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.04)] backdrop-blur-xl">
              {/* Top-edge sheen */}
              ...
              <div className="flex items-end gap-2 px-4 py-3">
                ...
                <div className="mb-0.5 flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg border border-cyan-500/40 bg-gradient-to-br from-slate-700 to-slate-800 shadow-[0_0_8px_rgba(6,182,212,0.2)]">
                  <svg viewBox="0 0 20 20" fill="none" className="h-4 w-4">
                    <circle cx="10" cy="10" r="8" stroke="currentColor" strokeWidth="1.5" className="text-cyan-500/40"/>
                    <circle cx="10" cy="10" r="2.5" fill="currentColor" className="text-cyan-400"/>
                  </svg>
                ...
                {speechSupported && (
                  <button
                    ...
                    className={`mb-0.5 flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg transition-colors ${
                      listening
                        ? "animate-pulse bg-red-500/30 text-red-400 border border-red-500/50"
                ...
                <button
                  ...
                  className="mb-0.5 flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-xl bg-cyan-500 text-slate-950 shadow-[0_0_12px_rgba(6,182,212,0.3)] transition-all hover:bg-cyan-400 hover:shadow-[0_0_16px_rgba(6,182,212,0.5)] disabled:cursor-not-allowed disabled:opacity-30 disabled:shadow-none"
                >
                  <svg viewBox="0 0 20 20" fill="currentColor" className="h-4 w-4">
              ...
              <div className="flex items-center justify-between px-4 pb-2.5">
                {speechSupported ? (
                  <select
                    ...
                    className="rounded-md border border-slate-700/60 bg-transparent px-1.5 py-0.5 text-[10px] text-slate-500 outline-none"
                  >
                ...
                <p className="text-[10px] text-slate-600">
                  ...
                  <kbd className="rounded border border-slate-700 px-1 font-mono">Shift+Enter</kbd> for new line
                </p>
              </div>
            </div>
          </div>
      ...
      {clearConfirm && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm">
          <div className="w-80 rounded-2xl border border-slate-700 bg-slate-900 p-6 shadow-2xl">
            <h3 className="text-sm font-semibold text-slate-100">Clear all conversations?</h3>
            <p className="mt-1 text-xs text-slate-500">This permanently deletes all your chat sessions.</p>
            <div className="mt-4 flex gap-2">
              <button onClick={() => setClearConfirm(false)} className="flex-1 rounded-lg border border-slate-700 py-2 text-xs text-slate-400 hover:bg-slate-800">Cancel</button>
