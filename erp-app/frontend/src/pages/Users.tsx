...
function roleTone(r: string) {
  ...
  if (r === "vendor") return "blue" as const;
          ...
          {visible.map((u) => (
            <Card key={u.username} className="flex flex-col gap-2 p-4">
              <div className="flex items-center justify-between gap-2">
                <span className="font-semibold text-slate-900 dark:text-slate-100">{u.display_name}</span>
                <Badge tone={roleTone(u.role)} dot>
                  {u.role}
                </Badge>
              </div>
              <span className="text-xs text-slate-500">@{u.username}</span>
              {u.company_name && <span className="text-xs text-slate-500">{u.company_name}</span>}
